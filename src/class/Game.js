...
import { CARD_WIDTH, CARD_OFFSET, HAND_WIDTH, HAND_HEIGH, HAND_RECT_X, HAND_RECT_Y, CARD_HEIGH } from '../config.js';
...
class Game {
    ...
    Start() {
        ...
        rectangle.beginFill(0xFFFFFF)
            .drawRect(HAND_RECT_X, HAND_RECT_Y, HAND_WIDTH, HAND_HEIGH)
            .endFill();
        ...
        cardLeftCntText.position.set(
            ...
            this.app.screen.height / 4 + CARD_HEIGH / 1.5
        );
        this.backCardContainer.addChildAt(cardLeftCntText, 0);

        const closedBackCard = new Card(1000, "0", 'card-shirt-1.png');
        closedBackCard.cardSprite.position.set(this.app.screen.width / 2, this.app.screen.height / 4 - 10);
        ...
        closedCard.cardSprite.position.set(this.app.screen.width / 2, this.app.screen.height / 4);
        ...
        if (element.cardSprite.data) {
            ...
            if (element.pointInRect({ x1, y1, x2, y2 }, { x, y })) {
                ...
                if (this.table.deck.cards.length == 0) {
                    ...
                    element.cardSprite.position.set(this.app.screen.width / 2, this.app.screen.height / 4);
                }
                this.Redraw();
            } else {
                element.cardSprite.position.set(this.app.screen.width / 2, this.app.screen.height / 4);
    ...
    Redraw() {
        ...
        player.sort((a, b) => a.card.id - b.card.id)
            .map((card, key) => {
                ...
                const yPos = HAND_RECT_Y + HAND_HEIGH / 2;
                ...
                scoreCntText.position.set(xPos, yPos + CARD_HEIGH / 1.5);
